...
| Primitive | Name | Why |
|---|---|---|
| tool | `ctxr_process` | Ingest ids, a playlist, or every video on a page. Inline for a few videos with per-video progress notifications; `background: true` returns immediately for big batches. |
